...
def main():
    ...
    meta = metainfo.MetaInfo('test.torrent')
    host_id = metainfo.hash('user@host')
    ...
    conn = connection.Connection(socket, ('127.0.0.1', 51413), timeout=60)

    protocol = PeerProtocol(host_id, data, conn=conn)
    protocol.loop()
